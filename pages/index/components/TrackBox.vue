<template>
  ...
  >
    <div class="w-[10%] text-gray-100 font-thin flex flex-row justify-center align-center items-center lg:pe-10">
      <p class="text-sm md:text-lg text-gray-400 dark:text-zinc-100 me-10">{{ num + 1}}</p>
      <font-awesome
        :icon="['fas', 'pause']"
        color="#8d8484"
    ...
    <div class="w-[20%] text-slate-800 dark:text-slate-200 font-thin hover:font-normal text-sm md:text-md cursor-pointer me-4 xl:pe-2 flex flex-wrap">
      <p @click="musicStore.play(props.track, props.index)">{{ props.track.title }}</p>
    </div>

    <div class="w-[20%] ctr-album text-slate-800 dark:text-slate-200 font-thin text-sm md:text-md">
      <p
        @click="musicStore.doFilter('album', props.track.album)"
        class="hover:font-normal cursor-pointer"
      >{{ props.track.album.title }}</p>
    </div>
    <div class="w-[20%] ctr-genres text-slate-800 dark:text-slate-200 font-thin text-sm md:text-md flex flex-wrap pe-5">
      <p 
    ...
    <div class="ctr-moods w-[20%] text-slate-800 dark:text-slate-200 font-thin text-sm md:text-md flex flex-wrap cursor-pointer">
      <p 
